...
final class ViewControllerAssembly: Assembly {
    func assemble(container: Container) {
        ...
        container.register(NewWalletViewController.self) { resolver in
            let vc = Storyboard.NewWallet.instantiate(NewWalletViewController.self)
            vc.viewModel = resolver.resolve(NewWalletViewModel.self)!
            return vc
        }

        container.register(WalletHistoryViewController.self) { (resolver, index: Int) in
            let vc = Storyboard.WalletHistory.instantiate(WalletHistoryViewController.self)
            vc.viewModel = resolver.resolve(WalletHistoryViewModel.self, argument: index)!
            return vc
        }
        ...
        container.register(WalletInfoViewController.self) { (resolver, index: Int) in
            let vc = Storyboard.WalletInfo.instantiate(WalletInfoViewController.self)
            vc.viewModel = resolver.resolve(WalletInfoViewModel.self, argument: index)!
            return vc
        }
